  ...
  var PS4IP = document.getElementById("psip").value;
    ...
    xhr.open("POST", `http://${PS4IP}:9090/status`);
		...
		xhr.onerror = function(){
			alert("¡¡ERROR AL CARGAR PAYLOAD!! Primero habilite el servidor Binloader en la configuración de GOLD-HEN");
			return;
		};
		xhr.onload = function(){
		  ...
		  getPayload(PLfile, function (xhr) {
           ...
           sendPayload(`http://${PS4IP}:9090`, xhr.response, function (xhr) {
            if (xhr.status === 200) {
              ...
              alert("¡¡Cargando Payload!!");
					   }else{
               alert("No se puede cargar payload seleccionado.");
               return;
              }
					})
				}
				...
				alert("No se puede cargar payload porque el servidor Binloader está ocupado!!");
				return;
			}
		};
	}
